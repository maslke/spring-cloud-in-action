...
import com.maslke.spring.demos.zuulsvr.util.AbTestingRoute;
...
public class SpecialRoutesFilter extends ZuulFilter {
    ...
    private String buildRouteString(String oldEndpoint, String newEndpoint, String serviceName) {
        ...
        String stippedRoute = oldEndpoint.substring(index + serviceName.length());
        String route = String.format("%s/%s", newEndpoint, stippedRoute);
        logger.info("Target route:{}", route);
        return route;
    }
    ...
    private HttpHost getHttpHost(URL host) {
        return new HttpHost(host.getHost(),
                host.getPort(), host.getProtocol());
    }
    ...
    public Object run() throws ZuulException {
        ...
        // 1.调用licensingservice
        // 2.licensingservice中会调用organizatonservice(调用的地址是，经过zuulservice的地址)
        // 3.在调用的时候，当前的RequestContext中的serviceId是organizationservice
        AbTestingRoute abTestingRoute = getAbRoutingInfo(filterUtils.getServiceId()); // 组织服务的记录 filterUtils中的serviceId是从RequestContext中获取的。
        if (abTestingRoute != null && useSpecialRoute(abTestingRoute)) {
            String route = buildRouteString(context.getRequest().getRequestURI(), abTestingRoute.getEndpoint(),
    ...
    private AbTestingRoute getAbRoutingInfo(String serviceName) {
        ...
        try {
            // 这个地方调用的是specialrouteservice的请求，没有经过zuul
            restExchange = restTemplate.exchange("http://specialroutesservice/v1/route/abtesting/{serviceName}", HttpMethod.GET,
